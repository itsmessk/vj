...
if (isset($_POST['add_to_cart'])) {
    ...
    $checkCartQuery = mysqli_query($con, "SELECT * FROM orders WHERE PId='$productId' AND UserId='$userId' AND IsOrderPlaced IS NULL");
    
    if (mysqli_num_rows($checkCartQuery) > 0) {
        // If the product is already in the cart, update the quantity
        $updateCartQuery = mysqli_query($con, "UPDATE orders SET quantity = quantity + 1 WHERE PId='$productId' AND UserId='$userId' AND IsOrderPlaced IS NULL");
        if ($updateCartQuery) {
            echo "<script>alert('Product quantity updated in the cart.');</script>";
        } else {
            echo "<script>alert('Error updating the cart. Please try again.');</script>";
        }
    } else {
        // If the product is not in the cart, add it
        $addCartQuery = mysqli_query($con, "INSERT INTO orders (PId, UserId, quantity) VALUES ('$productId', '$userId', 1)");
        if ($addCartQuery) {
            echo "<script>alert('Product added to the cart.');</script>";
        } else {
            echo "<script>alert('Error adding the product to the cart. Please try again.');</script>";
        }
    }
